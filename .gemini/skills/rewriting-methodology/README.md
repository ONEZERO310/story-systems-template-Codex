# Rewriting Methodology

![Status](https://img.shields.io/badge/status-production-green)
![Harness](https://img.shields.io/badge/harness-v1.5-blue)
![Provider](https://img.shields.io/badge/provider-Gemini_CLI-orange)

> WTFB 6-step screenplay rewriting process.

## Attribution

**Copyright:** (c) 2026 J. Scott Graham ([@cheddarfox](https://github.com/cheddarfox)) / [Bybren LLC](https://github.com/bybren-llc)

**Attribution:** Required per [/NOTICE](/NOTICE)

## Quick Start

This skill activates automatically when you:

- Begin the rewrite process
- Identify weak scenes
- Improve overall script quality
- Prepare for final polish

## What This Skill Does

Provides the WTFB 6-step screenplay rewriting process covering identifying
story threads, evaluating scenes, improving dialogue, polishing action, and
preparing for final draft submission.

## Activation Triggers

Extracted from skill description:

| Trigger |
|---------|
| beginning rewrite process |
| identifying weak scenes |
| improving script quality |
| preparing for final polish |

## Related Skills

- [continuity-tracking](../continuity-tracking/) - Consistency systems
- [page-estimation](../page-estimation/) - Page count calculation
- [format-export](../format-export/) - Export procedures

## Provider Compatibility

| Provider | Status |
|----------|--------|
| Gemini CLI | ✅ Native |
| Codex | ✅ Equivalent skill in `codex/skills/` |

## Quality Checklist

- [x] `name:` matches directory name
- [x] `description:` includes "Use when:" clause
- [x] Trigger conditions are specific
- [x] README summary matches frontmatter description

## Maintenance

| Field | Value |
|-------|-------|
| Last Updated | 2026-01-14 |

---

*Full implementation details in [SKILL.md](SKILL.md)*
