# Title Page

![Status](https://img.shields.io/badge/status-production-green)
![Harness](https://img.shields.io/badge/harness-v1.5-blue)
![Provider](https://img.shields.io/badge/provider-Gemini_CLI-orange)

> Fountain title page formatting and metadata setup.

## Attribution

**Copyright:** (c) 2026 J. Scott Graham ([@cheddarfox](https://github.com/cheddarfox)) / [Bybren LLC](https://github.com/bybren-llc)

**Attribution:** Required per [/NOTICE](/NOTICE)

## Quick Start

This skill activates automatically when you:

- Create new screenplays
- Set up title page metadata
- Update screenplay information
- Prepare scripts for submission

## What This Skill Does

Provides Fountain title page formatting and metadata setup covering required
fields (title, author, contact), optional fields (copyright, draft date, notes),
and submission-ready formatting.

## Activation Triggers

Extracted from skill description:

| Trigger |
|---------|
| creating new screenplays |
| setting up title page metadata |
| updating screenplay information |
| preparing scripts for submission |

## Related Skills

- [fountain-syntax](../fountain-syntax/) - Complete Fountain format reference
- [character-dialogue](../character-dialogue/) - Character and dialogue formatting
- [scene-headings](../scene-headings/) - Scene heading formatting rules

## Provider Compatibility

| Provider | Status |
|----------|--------|
| Gemini CLI | ✅ Native |
| Codex | ✅ Equivalent skill in `codex/skills/` |

## Quality Checklist

- [x] `name:` matches directory name
- [x] `description:` includes "Use when:" clause
- [x] Trigger conditions are specific
- [x] README summary matches frontmatter description

## Maintenance

| Field | Value |
|-------|-------|
| Last Updated | 2026-01-14 |

---

*Full implementation details in [SKILL.md](SKILL.md)*
