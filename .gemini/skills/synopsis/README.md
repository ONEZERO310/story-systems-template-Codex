# Synopsis

![Status](https://img.shields.io/badge/status-production-green)
![Harness](https://img.shields.io/badge/harness-v1.5-blue)
![Provider](https://img.shields.io/badge/provider-Gemini_CLI-orange)

> One-page synopsis writing techniques for screenplays.

## Attribution

**Copyright:** (c) 2026 J. Scott Graham ([@cheddarfox](https://github.com/cheddarfox)) / [Bybren LLC](https://github.com/bybren-llc)

**Attribution:** Required per [/NOTICE](/NOTICE)

## Quick Start

This skill activates automatically when you:

- Prepare submission materials
- Create one-page story summaries
- Pitch to producers or executives
- Condense screenplay for coverage

## What This Skill Does

Provides one-page synopsis writing techniques for screenplays covering the
five requirements of a good synopsis, synopsis vs treatment comparison, and
formatting for submission to producers and executives.

## Activation Triggers

Extracted from skill description:

| Trigger |
|---------|
| preparing submission materials |
| creating story summaries |
| pitching to producers |
| condensing screenplay |

## Related Skills

- [logline](../logline/) - Logline writing techniques
- [pitch-worksheet](../pitch-worksheet/) - Pitch development framework
- [theme-discovery](../theme-discovery/) - Personal theme mining

## Provider Compatibility

| Provider | Status |
|----------|--------|
| Gemini CLI | ✅ Native |
| Codex | ✅ Equivalent skill in `codex/skills/` |

## Quality Checklist

- [x] `name:` matches directory name
- [x] `description:` includes "Use when:" clause
- [x] Trigger conditions are specific
- [x] README summary matches frontmatter description

## Maintenance

| Field | Value |
|-------|-------|
| Last Updated | 2026-01-14 |

---

*Full implementation details in [SKILL.md](SKILL.md)*
